...
Le modèle documentaire ajoute deux axes de métadonnées indépendants. Le domaine fonctionnel couvre `PREVENTION`, `AT_MP`, `AIDES_PREVENTION`, `ACCOMPAGNEMENT_ENTREPRISE`, `RETRAITE`, `STATISTIQUES`, `OUTILS`, `GUIDES` et `AUTRE`. La catégorie documentaire couvre guide, fiche, brochure, dossier, formulaire, outil, publication, statistique, FAQ et autre.

Les deux axes valent `AUTRE` par défaut : les constructions LOT 0 sans domaine fonctionnel ni catégorie restent donc valides et conservent leur empreinte historique. La sérialisation inclut explicitement ces métadonnées ; la désérialisation accepte les anciens dictionnaires qui ne les contiennent pas. Connector Platform demeure la source unique de vérité pour tous les éléments du contrat et aucune capacité opérationnelle n'est ajoutée.
